...
class GenerateTestUserSig {
    ...
    class func getSafeUrl(_ streamName: String) -> String {
        
        let txTime = Int(Date().timeIntervalSince1970) + 60 * 60
        var input = LIVE_URL_KEY
        input.append(streamName)
        input.append(txTime.toHexString().uppercased())
        
        let txSecret = input.md5String(use: .utf8)
        return "?txSecret=\(txSecret)&txTime=\(txTime.toHexString())"
    }
}
...
fileprivate extension String {
    func md5String(use: String.Encoding) -> String {
        let digest = Insecure.MD5.hash(data: data(using: use) ?? Data())
        return digest.map {
