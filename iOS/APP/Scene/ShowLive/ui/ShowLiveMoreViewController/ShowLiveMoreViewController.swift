...
class ShowLiveMoreViewController: UIViewController {
    ...
    private lazy var collectionLayout: UICollectionViewFlowLayout = {
        ...
        let contentWidth = ScreenWidth - 40 - 10 * 3
        if MLVBConfigManager.shared.isSetupService {
            let width = floor(contentWidth / 2) - 1
            layout.itemSize = CGSize(width: width, height: 1.2 * width)
        } else {
            layout.itemSize = CGSize(width: contentWidth, height: 300)
        }
        return layout
    }()
    ...
    private func joinRoom(_ roomId: String) {
        IMRoomManager.sharedManager().getIMRoomInfoList(roomIds: [roomId]) { [weak self] (code, message, roomInfos) in
            ...
            if let roomInfo = roomInfos.first {
                self.finishInteractiveTransition { [weak self] in
                    guard let self = self else { return }
...
extension ShowLiveMoreViewController: UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard MLVBConfigManager.shared.isSetupService else {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ShowLiveLiteCollectionCell.reuseIdentifier, for: indexPath) as! ShowLiveLiteCollectionCell
            cell.joinRoomBlock = { [weak self] (roomId) in
                guard let self = self else { return }
